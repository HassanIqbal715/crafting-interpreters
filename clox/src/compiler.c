...
typedef struct {
    ...
    Token previous;
    bool hadError;
    bool panicMode; // skip errors when parser doesn't know what it's parsing.
} Parser;
...
static void errorAt(Token* token, const char* message) {
    if (parser.panicMode) return; // do not report any error during panic mode
...
static void emitLoop(int loopStart) {
    ...
    // +2 because of OP_LOOP's 2byte-long operands
...
static int emitJump(uint8_t instruction) {
    ...
    emitByte(0xff);
    emitByte(0xff);
    return currentChunk()->count - 2; // start of operand.
}
...
static uint32_t makeConstant(Value value) {
    ...
    uint32_t result = (uint32_t)constant;
...
static void emitConstant(Value value) {
    ...
    if (!IS_LONG(constant)) {
        ...
        emitBytes(OP_CONSTANT_LONG, (uint8_t)(constant >> 16));
        emitBytes((uint8_t)(constant >> 8), (uint8_t)constant);
    }
}

static void patchJump(int offset) {
    // This number tells how many instructions to jump.
    // currenChunk()->count - offset gives the total instructions from the start
    // of operand (inclusive). The -2 skips the whole operand and gives the
    // jump count from start of if's body till its end.
    ...
    // store count as jump instruction's operand
...
static void writeLocalArray(LocalArray* array, 
    ...
    array->locals[array->count].isCaptured = isCaptured;
    array->count++;
}
...
static void initCompiler(Compiler* compiler, FunctionType type) {
    ...
    name.start = "";
    name.length = 0;
    writeLocalArray(&compiler->locals, name, 0, false);
}
...
static void endScope() {
    ...
    while (current->locals.count > 0 && 
        current->locals.locals[current->locals.count - 1].depth >
            current->scopeDepth) {
        if (current->locals.locals[current->locals.count - 1].isCaptured) {
...
static int resolveUpvalue(Compiler* compiler, Token* name) {
    ...
    if (local != -1) {
        compiler->enclosing->locals.locals[local].isCaptured = true;
...
static void addLocal(Token name) {
    ...
    writeLocalArray(&current->locals, name, -1, false);
}
...
static void defineVariable(uint32_t global) {
    ...
    if (!IS_LONG(global)) {
        ...
        emitBytes(OP_DEFINE_GLOBAL_LONG, (uint8_t)(global >> 16));
        emitBytes((uint8_t)(global >> 8), (uint8_t)global);
    }
}
...
static void and_(bool canAssign) {
    ...
    patchJump(endJump); // jump if true, else check the other operands.
}
...
static void or_(bool canAssign) {
    ...
    patchJump(elseJump); // jump over the endJump if false.
    emitByte(OP_POP);

    parsePrecedence(PREC_OR);
    patchJump(endJump); // true, jump to the end of all operands.
}
...
static void forStatement() {
    ...
    if (!match(TOKEN_SEMICOLON)) {
        expression();
        consume(TOKEN_SEMICOLON, "Expect ';' after after loop condition.");
    ...
    if (exitJump != -1) { // condition exists, calculate jump offset
        patchJump(exitJump);
        emitByte(OP_POP); // Condition.
    }
...
static void ifStatement() {
    ...
    patchJump(thenJump); // end of if.

    emitByte(OP_POP);

    if (match(TOKEN_ELSE)) statement();
    patchJump(elseJump); // helps 'then' branch jump 'else' branch.
}
...
static void unary(bool canAssign) {
    ...
    // Compile the operand that follows this operator
...
static void parsePrecedence(Precedence precedence) {
    ...
    // First token will always be a prefix. Negation, number, not.
    if (prefixRule == NULL) {
        error("Expect expression.");
        return;
    }

    // only assign if precedence is lower than or equal to assignment
    bool canAssign = precedence <= PREC_ASSIGNMENT;
    prefixRule(canAssign);

    // All tokens of the same or higher precedence as the enum passed are
    // parsed
